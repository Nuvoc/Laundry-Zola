...
const AdminPage = (() => {
  ...
  function renderLogin(container) {
    container.innerHTML = `
      <div class="admin-login-container" style="display:flex;align-items:center;justify-content:center;min-height:100vh;background:var(--bg-primary);">
        <div class="card" style="max-width:400px;width:100%;padding:2rem;box-shadow:0 10px 25px rgba(0,0,0,0.2);">
          ...
          <form id="admin-login-form">
            <div class="form-group">
              ...
              <input type="text" id="admin-username" class="form-input" required placeholder="Masukkan username">
            </div>
            <div class="form-group">
              <label class="form-label">Password</label>
              <input type="password" id="admin-password" class="form-input" required placeholder="Masukkan password">
            </div>
            <button type="submit" class="btn btn-primary" style="width:100%;margin-top:1rem;">Login</button>
            <button type="button" class="btn btn-secondary" style="width:100%;margin-top:0.5rem;" onclick="Router.navigate('/user')">Kembali ke Halaman User</button>
          </form>
        </div>
      </div>
    `;
    ...
    return `
      <aside class="sidebar" id="admin-sidebar">
        ...
        <div class="sidebar-footer">
          ...
          <a class="nav-item" style="color:var(--accent-danger);margin-top:0.5rem;cursor:pointer;" onclick="AdminPage.logout()">
    ...
    container.innerHTML = `
      <div class="admin-page-header">
        <div>
          ...
          <p class="admin-page-subtitle">Rincian pendapatan dari transaksi selesai dan diambil</p>
        </div>
      </div>
      <div class="card" style="margin-bottom: 2rem;">
        <div id="revenue-summary" style="padding: 1rem;">
          ${Components.skeletonStats(1)}
        </div>
      </div>
      <div class="card">
        <div class="table-toolbar">
          <div class="filter-group">
            <h3 style="margin:0;font-size:1rem;">Riwayat Transaksi (Selesai & Diambil)</h3>
          </div>
        </div>
  ...
  async function loadRevenue() {
    ...
    if (cachedData.transaksi && cachedData.transaksi.length > 0) {
      displayRevenue(cachedData.transaksi);
    } else {
      try {
        const result = await API.Transaksi.getAll();
        if (result.success) {
  ...
  function displayRevenue(data) {
    ...
    const totalRevenue = revenueData.reduce((sum, t) => sum + (Number(t.total_harga) || 0), 0);
    ...
    if (summaryEl) {
      summaryEl.innerHTML = `
        <div style="display:flex;align-items:center;gap:1.5rem;">
          ...
          <div>
            <div style="color:var(--text-tertiary);font-size:0.875rem;margin-bottom:0.25rem;">Total Pendapatan (Selesai & Diambil)</div>
            <div style="font-size:1.5rem;font-weight:700;color:var(--accent-success);">${Components.formatCurrency(totalRevenue)}</div>
            <div style="color:var(--text-secondary);font-size:0.875rem;margin-top:0.25rem;">Dari ${revenueData.length} transaksi</div>
          </div>
        </div>
      `;
    }
    ...
    if (revenueData.length === 0) {
      tableEl.innerHTML = Components.emptyState('💸', 'Belum Ada Pendapatan', 'Belum ada transaksi yang selesai atau diambil');
      return;
    }
    ...
    revenueData.forEach(t => {
      html += `
        <tr>
          ...
          <td>${t.nama_layanan} (${t.berat_kg} kg)</td>
          <td style="font-size:var(--text-xs)">${Components.formatDate(t.tanggal_masuk)}</td>
          <td>${Components.statusBadge(t.status)}</td>
          <td style="font-weight:600;color:var(--accent-success)">${Components.formatCurrency(t.total_harga)}</td>
        </tr>`;
    });
  ...
  async function renderBiaya(container) {
    container.innerHTML = `
      <div class="admin-page-header">
        <div>
          <h1 class="admin-page-title">Manajemen Biaya</h1>
          <p class="admin-page-subtitle">Kelola pengeluaran operasional laundry</p>
        </div>
        <button class="btn btn-primary" onclick="AdminPage.showAddBiaya()">
          + Tambah Biaya
        </button>
      </div>
      <div class="stats-grid" id="biaya-stats" style="margin-bottom: var(--space-6)">
        ${Components.skeletonStats(1)}
      </div>
      <div class="card">
        <div id="biaya-table">
  ...
  function loadExpenses() {
    const expenses = JSON.parse(localStorage.getItem('laundry_expenses') || '[]');
    displayExpenses(expenses);
  }

  function displayExpenses(data) {
    const totalBiaya = data.reduce((sum, item) => sum + Number(item.jumlah), 0);

    const statsEl = document.getElementById('biaya-stats');
    if (statsEl) {
      statsEl.innerHTML = Components.statsCard('💸', Components.formatCurrency(totalBiaya), 'Total Pengeluaran', 'stat-expenses');
    }

    const tableEl = document.getElementById('biaya-table');
    if (!tableEl) return;

    if (data.length === 0) {
      tableEl.innerHTML = Components.emptyState('💸', 'Belum Ada Biaya', 'Catatan pengeluaran akan muncul di sini');
      return;
    }
    ...
    data.forEach((item, index) => {
      html += `
        <tr>
          <td>${Components.formatDate(item.tanggal)}</td>
          <td style="color:var(--text-primary);font-weight:500">${item.keterangan}</td>
          <td><span class="badge badge-antrian">${item.kategori}</span></td>
          <td style="font-weight:600;color:var(--accent-danger)">${Components.formatCurrency(item.jumlah)}</td>
          <td>
            <div class="table-actions">
              <button class="btn btn-ghost btn-icon" title="Hapus" onclick="AdminPage.deleteExpense(${item.id})">🗑️</button>
            </div>
          </td>
        </tr>`;
    });
  ...
  function showAddBiaya() {
    const body = `
      <form id="form-add-biaya">
        <div class="form-group">
          <label class="form-label">Keterangan</label>
          <input type="text" class="form-input" id="exp-keterangan" required placeholder="Contoh: Beli Sabun, Bayar Listrik">
        </div>
        <div class="form-group">
          <label class="form-label">Jumlah (Rp)</label>
          <input type="number" class="form-input" id="exp-jumlah" required placeholder="10000">
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          ...
          <input type="date" class="form-input" id="exp-tanggal" value="${new Date().toISOString().split('T')[0]}" required>
        </div>
      </form>`;
  ...
  function deleteExpense(id) {
    Components.confirm('Yakin ingin menghapus catatan biaya ini?', () => {
      let expenses = JSON.parse(localStorage.getItem('laundry_expenses') || '[]');
  ...
  function logout() {
    ...
    render();
    Components.toast('Berhasil logout', 'success');
  }
